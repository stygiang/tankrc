...
- Motor drivers: pair of TB6612FNG dual H-bridge boards; each board carries one track and exposes PWMA/B+IN1/2 pins plus STBY as defined in `config/hardware_map.h`.
- RC input: 6-channel PWM receiver (steering/throttle/button/mode switch + two ultrasonic channels) wired to the pins listed in `config/hardware_map.h` and adjustable via the serial wizard.
...
- Networking: ESP32 station mode + fallback AP (`sharc`) so you can access the web control panel even away from infrastructure Wi-Fi.
...
- Hardware reference: the canonical board/pin map now lives in `config/hardware_map.h` (see `hardware_layout.h` for the metadata schema). Add new boards there so the UI/config tooling can surface physical pinouts alongside the runtime assignments.
